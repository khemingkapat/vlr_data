...
class Match:
    def __init__(self, node) -> None:
        ...
        match_name = (
            self.html.css_first("title").text().strip().split(" | ")[0].split(" vs. ")
        )
        ...
        self.stage = re_strip(
            " ", self.html.css_first("div.match-header-event-series").text()
        )

        # self.ban_pick = re_strip(' ',self.html.css_first('div.match-header-note').text()).split('; ')
        self.ban_pick = (
            None
            if (h := self.html.css_first("div.match-header-note")) is None
    ...
    def _extract_df(self, df):
        ...
        formatted_df = filled_df.rename(
            columns={"Unnamed: 0": "Name", "Unnamed: 1": "Team"}
        )
        ...
        for nc, c in zip(new_columns, formatted_df.iloc[:, 2:].columns):
            result_df[list(map(lambda x: tuple(x), list(nc)))] = formatted_df[
                c
            ].str.split(" ", expand=True)

        result_df.columns = pd.MultiIndex.from_tuples(
            result_df.columns, name=["Type", "Side"]
        )
    ...
    def get_scoreboard(self):
        ...
        for m in maps:
            ...
            match_header = (
                "Overall"
                if not current_map
                else " ".join(re_strip(" ", current_map.text()).split(" ")[::2])
            )
